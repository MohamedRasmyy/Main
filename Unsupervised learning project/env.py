...
class CartPole:
    def __init__(self,physics):
        ...
        self.length=0.7
        self.pole_mass_length= self.math_pole * self.length
    ...
    def show_cart(self, state_tuple, pause_time):
        ...
        ax.set_ylim(-0.5, 3.5)
        ax.plot(X, Y)
        cart = patches.Rectangle((x - 0.4, -0.25), 0.8, 0.25,linewidth=1, edgecolor='k', facecolor='cyan')
        base = patches.Rectangle((x - 0.01, -0.5), 0.02, 0.25,linewidth=1, edgecolor='k', facecolor='r')
        ax.add_patch(cart)
        ax.add_patch(base)
        x_dot_str, theta_str, theta_dot_str = '\\dot{x}', '\\theta', '\\dot{\\theta}'
        ax.set_title('x: %.3f, $%s$: %.3f, $%s$: %.3f, $%s$: %.3f'                                %(x, x_dot_str, x_dot, theta_str, theta, theta_dot_str, x))
        plt.show()
